...
namespace Utils
{
    public sealed class LoggerHelper
    {
        ...
        {
            string debugPath = PathUtils.GetPersistentPath("debug.data");

            IsOpenDebug = File.Exists(debugPath);

            string writeStrPath = PathUtils.GetPersistentPath("writeStr.data");
            if (!IsOpenWriteStrToFile)
            {
                IsOpenWriteStrToFile = File.Exists(writeStrPath);
            }

            string writeLogPath = PathUtils.GetPersistentPath("writeLog.data");
            if (!IsOpenWriteLogToFile && File.Exists(writeLogPath))
            {
                IsOpenWriteLogToFile = File.Exists(writeLogPath);
            }
        }
